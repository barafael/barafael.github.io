...
In short, the article proposes an architectural style which relies on
...
Alice's post had a strong influence on me.
It shaped and will continue to shape the asynchronous designs I take part in.
Unsurprisingly, I'm not the only one - everybody working with Rust in network applications seems to relate.
...
Alice's post starts off with a simple recipe, briefly introducing the style by example.
Although the principle remains, I suggest a few changes.

### I believe that an actor should *be* its data

In the original recipe, the actor holds its state and its receiver.
...
It may require them as arguments to its event loop of course! But not hold them as members.
...
By returning `Self`, we can inspect the state after the event loop finished, and maybe restart it.
Or, we can assert that the correct target state has been reached in a [unit test](#simple-unit-test).

### I like to leave the spawning to the user

The original recipe [`tokio::spawn`]s the actor in a helper function.
Who are we to prescribe that particular use? The user may want to
...
I.e., the actor stays alive until it realizes it cannot be useful anymore somehow.
Then, it breaks from the event loop, [dropping all runtime resources](#automatic-application-shutdown).
...
> **_Note:_** An actor should generally detect the condition where it is no longer needed and shut down by breaking out of its event loop. This is preferrable to explicitly shutting down an actor. Ideally, shutdown naturally happens gracefully when each actor finishes processing and drops its actor handles. Some designs do require to distribute a [`tokio_util::sync::CancellationToken`](https://docs.rs/tokio-util/latest/tokio_util/sync/struct.CancellationToken.html) among the components, though.

> **_More Note:_** `std::ops::drop` runs when the event loop exits, which will drop at least the event loop resources. If the task was spawned and isn't being awaited somewhere, the full actor resources are dropped.

> **_Even More Note:_** Sounds like garbage collection? [Not a coincidence](#ooop).
...
But don't fold in the `oneshot::Receiver` like in the original handle implementation!
Then, you can collect the oneshot returned here and await it at your convenience.
For an example, see the [unit test](#simple-unit-test).


### <a name="simple-unit-test"></a> Unit Testing the `UniqueIdService`
...
Care must be taken to not run into timing-dependent behaviour in a unit test (and **please avoid at great cost situations where you put a `sleep` call in your test! Major code reek**).
...
After this, when we `await` the actor event loop, we encounter "natural actor shutdown", and then assert on the result.
...
## <a name="bounded-mpsc-footgun"></a>Ruling out Deadlocks with Bounded Channels
...
This situation is bad in another way.
The actor cycle would prevent natural actor shutdown of A and B, because there always exists a handle.
...
## <a name="automatic-application-shutdown"></a>Automatic Application Shutdown
...
# <a name="ooop"></a>This style facilitates "Original OOP" (OOOP)
